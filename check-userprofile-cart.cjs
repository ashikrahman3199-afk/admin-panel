...
async function run() {
    ...
    let usersWithCart = res.Items.filter(u => Object.keys(u).some(k => k.toLowerCase().includes('cart')));
    console.log("Users with cart field:", JSON.stringify(usersWithCart, null, 2));
}
run();
